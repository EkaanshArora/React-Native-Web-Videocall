...
import { View, StyleSheet, StatusBar, TouchableOpacity, FlatList, Text, Platform, Image, Dimensions, ScrollView } from 'react-native';
...
import MinUidContext, { MinUidConsumer } from './agora-rn-uikit/src/MinUidContext';
import MaxUidContext, { MaxUidConsumer } from './agora-rn-uikit/src/MaxUidContext';
...
const App: React.FC<PropsInterface> = () => {
  ...
  let data = useContext(MinUidContext);
  let data2 = useContext(MaxUidContext);
  console.log(data, data2)
  let users = [...data, ...data2];
  return (videoCall ?
            ...
            <View style={{ flex: 1 }}>
              ...
              <View style={{ flex: 2, backgroundColor: '#121116', flexDirection: 'row' }}>
                <View style={{ width: width / 7.5, height: width / 7.5, backgroundColor: '#444', borderRadius: width / 6, alignItems: 'center', marginHorizontal: 15, alignSelf: 'center' }}>
                  <MinUidConsumer>
                    {minUsers => (
                      <Text style={{ fontSize: 22, lineHeight: width / 7.5, color: '#ABABAB', }}>+{minUsers.length}</Text>
                    )}
                  </MinUidConsumer>
                </View>
                    ...
                    {(data) => (
                      <MinUidConsumer>
                        {(minUsers) => minUsers.map((user) => (
                          <TouchableOpacity style={{ width: width / 4.5, borderRadius: 10, borderColor: '#fff', borderWidth: StyleSheet.hairlineWidth * 3, marginHorizontal: 2, marginVertical: 10, zIndex: 40 }}
